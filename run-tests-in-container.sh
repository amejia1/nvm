...
#
# Temporary log files (like the output of a run) should be written to a dedicated directory under "${HOME}/.cache"
# (like "${HOME}/.cache/ai_agent_scratch_space"), not to "/tmp": mount that directory into the container at the
# same location when logs need to be written, for example:
...
# the suite is retried (up to 3 attempts), as in the CI. The "installation_node" suite gets a larger default (4200
# seconds) on modern systems: without python 2 it builds a modern node release from source, which takes much longer
# than the v0.10.7 build that the CI's Ubuntu 16.04 job does. SUITE_TIMEOUT overrides the per-suite defaults.
#
...
# Per-suite timeout: SUITE_TIMEOUT (if set) overrides everything; otherwise "installation_node" gets a larger
# default (its node source builds are much slower on modern systems than in the CI, see above).
suite_timeout () {
  if [ -n "${SUITE_TIMEOUT:-}" ]; then
    echo "$SUITE_TIMEOUT"
  elif [ "$1" = installation_node ]; then
    echo 4200
  else
    echo 600
  fi
...
for suite in fast slow sourcing installation_node installation_iojs install_script; do
  ...
  while true; do
    ...
    timeout -k 30 "$(suite_timeout "$suite")" ./node_modules/.bin/urchin -f -s bash "test/$suite" || exit_code=$?
